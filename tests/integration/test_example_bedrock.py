"""Pin the Bedrock worked example to its expected outputs."""
...
EXAMPLE_DIR = Path(__file__).resolve().parents[2] / "examples" / "bedrock_basic_agent"


def _ingest_options() -> BedrockIngestOptions:
    return BedrockIngestOptions(
        architecture="single_agent",
        stack_tier=StackTier.WITHIN_STACK,
    )


def test_bedrock_ingest_produces_expected_manifest() -> None:
...
def test_bedrock_example_reconstructs_to_expected_feasibility() -> None:
    sessions = load_sessions_file(EXAMPLE_DIR / "input" / "sessions.json")
    manifest = sessions_to_manifest(
        sessions,
        scenario_id="bedrock_basic_agent_demo",
...
def test_bedrock_example_emits_expected_prov_jsonld() -> None:
    sessions = load_sessions_file(EXAMPLE_DIR / "input" / "sessions.json")
    manifest = sessions_to_manifest(
        sessions,
        scenario_id="bedrock_basic_agent_demo",
