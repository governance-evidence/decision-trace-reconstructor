"""Pin the OpenAI Agents worked example to its expected outputs."""
...
EXAMPLE_DIR = Path(__file__).resolve().parents[2] / "examples" / "openai_agents_basic_agent"


def _ingest_options() -> OpenAIAgentsIngestOptions:
    return OpenAIAgentsIngestOptions(
...
def test_openai_agents_ingest_produces_expected_manifest() -> None:
...
def test_openai_agents_example_reconstructs_to_expected_feasibility() -> None:
    traces = load_traces_file(EXAMPLE_DIR / "input" / "trace.json")
    manifest = trace_to_manifest(
        traces[0],
        scenario_id="openai_agents_basic_agent_demo",
...
def test_openai_agents_example_emits_expected_prov_jsonld() -> None:
    traces = load_traces_file(EXAMPLE_DIR / "input" / "trace.json")
    manifest = trace_to_manifest(
        traces[0],
        scenario_id="openai_agents_basic_agent_demo",
