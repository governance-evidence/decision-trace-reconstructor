"""Pin the Anthropic worked example to its expected outputs."""
...
EXAMPLE_DIR = Path(__file__).resolve().parents[2] / "examples" / "anthropic_basic_agent"


def _ingest_options() -> AnthropicIngestOptions:
    return AnthropicIngestOptions(
        architecture="single_agent",
        stack_tier=StackTier.WITHIN_STACK,
    )


def test_anthropic_ingest_produces_expected_manifest() -> None:
...
def test_anthropic_example_reconstructs_to_expected_feasibility() -> None:
    rounds = load_rounds_file(EXAMPLE_DIR / "input" / "messages_history.jsonl")
    manifest = rounds_to_manifest(
        rounds,
        scenario_id="anthropic_basic_agent_demo",
...
def test_anthropic_example_emits_expected_prov_jsonld() -> None:
    rounds = load_rounds_file(EXAMPLE_DIR / "input" / "messages_history.jsonl")
    manifest = rounds_to_manifest(
        rounds,
        scenario_id="anthropic_basic_agent_demo",
