"""Pin the CrewAI worked example to its expected outputs."""
...
from reconstructor.adapters.crewai import CrewAIIngestOptions, events_to_manifest, load_events_file
...
EXAMPLE_DIR = Path(__file__).resolve().parents[2] / "examples" / "crewai_basic_agent"


def _ingest_options() -> CrewAIIngestOptions:
    return CrewAIIngestOptions(
...
def test_crewai_ingest_produces_expected_manifest() -> None:
    events = load_events_file(EXAMPLE_DIR / "input" / "events.jsonl")
    manifest = events_to_manifest(
        events, scenario_id="crewai_basic_agent_demo", opts=_ingest_options()
    )
    expected = json.loads((EXAMPLE_DIR / "expected_output" / "fragments.json").read_text())
    assert manifest == expected


def test_crewai_example_reconstructs_to_expected_feasibility() -> None:
    events = load_events_file(EXAMPLE_DIR / "input" / "events.jsonl")
    manifest = events_to_manifest(
        events, scenario_id="crewai_basic_agent_demo", opts=_ingest_options()
    )
...
def test_crewai_example_emits_expected_prov_jsonld() -> None:
    events = load_events_file(EXAMPLE_DIR / "input" / "events.jsonl")
    manifest = events_to_manifest(
        events, scenario_id="crewai_basic_agent_demo", opts=_ingest_options()
    )
