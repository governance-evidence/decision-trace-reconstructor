"""Pin the Agent Framework worked example to its expected outputs."""
...
EXAMPLE_DIR = Path(__file__).resolve().parents[2] / "examples" / "agentframework_basic_agent"


def _ingest_options() -> AgentFrameworkIngestOptions:
    return AgentFrameworkIngestOptions(
...
def test_agentframework_ingest_produces_expected_manifest() -> None:
...
def test_agentframework_example_reconstructs_to_expected_feasibility() -> None:
    events = load_events_file(EXAMPLE_DIR / "input" / "events.jsonl")
    manifest = events_to_manifest(
        events,
        scenario_id="agentframework_basic_agent_demo",
...
def test_agentframework_example_emits_expected_prov_jsonld() -> None:
    events = load_events_file(EXAMPLE_DIR / "input" / "events.jsonl")
    manifest = events_to_manifest(
        events,
        scenario_id="agentframework_basic_agent_demo",
