"""Pin the MCP worked example to its expected outputs."""
...
EXAMPLE_DIR = Path(__file__).resolve().parents[2] / "examples" / "mcp_basic_agent"


def _ingest_options() -> McpIngestOptions:
    return McpIngestOptions(
...
def test_mcp_ingest_produces_expected_manifest() -> None:
    frames = load_transcript_file(EXAMPLE_DIR / "input" / "transcript.jsonl")
    manifest = transcript_to_manifest(
        frames,
...
def test_mcp_example_reconstructs_to_expected_feasibility() -> None:
    frames = load_transcript_file(EXAMPLE_DIR / "input" / "transcript.jsonl")
    manifest = transcript_to_manifest(
        frames,
        scenario_id="mcp_basic_agent_demo",
...
def test_mcp_example_emits_expected_prov_jsonld() -> None:
    frames = load_transcript_file(EXAMPLE_DIR / "input" / "transcript.jsonl")
    manifest = transcript_to_manifest(
        frames,
        scenario_id="mcp_basic_agent_demo",
